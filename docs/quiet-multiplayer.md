...
4. Realtime Database rules enforce a maximum of two active public presence records.
5. A private claim lets each traveler update only their own presence without exposing their account identity to the other participant.
...
- Room capacity, expiry, payload shape, and payload ranges are validated in Realtime Database rules.
